...
class BrnEnhanceNumberCardConfig extends BrnBaseConfig {
  ...
  BrnEnhanceNumberCardConfig({
    ...
    this.runningSpace = runningSpace ?? 16;
    this.itemRunningSpace = itemRunningSpace ?? 8;
    this.titleTextStyle = titleTextStyle ?? BrnTextStyle(fontSize: 28, fontWeight: FontWeight.w600);
    this.descTextStyle = descTextStyle ??
        BrnTextStyle(
            fontSize: 12, color: BrnDefaultConfigUtils.defaultCommonConfig.colorTextSecondary);
    this.dividerWidth = dividerWidth ?? 0.5;
  }

  ///如果超过一行，行间距
  late double runningSpace;

  ///Item的上半部分和下半部分的间距
  late double itemRunningSpace;

  late double dividerWidth;

  late BrnTextStyle titleTextStyle;

  late BrnTextStyle descTextStyle;
